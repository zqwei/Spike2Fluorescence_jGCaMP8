...
def run_single_cell(nf, model_list=model_list, model_name=model_name):
    nf = nf.squeeze()
    if os.path.exists('GCaMP8_exported_ROIs_s2f_full/fit_results_v01/'+nf+'_fit.npz'):
        return None
    ...
    param_est = []
    if os.path.exists('GCaMP8_exported_ROIs_s2f_full/fit_results_v00/'+nf+'_fit_v00.npz'):
        _=np.load('GCaMP8_exported_ROIs_s2f_full/fit_results_v00/'+nf+'_fit_v00.npz', allow_pickle=True)
    ...
    np.savez('GCaMP8_exported_ROIs_s2f_full/fit_results_v01/'+nf+'_fit', param_est=np.array(param_est))
    return None
