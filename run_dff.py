...
flist = sorted(glob('GCaMP8_exported_ROIs_s2f_full/data_v01/*.npz'))
cell_info=["_".join(os.path.basename(f).split('_')[:6]) for f in flist]
cell_info_uq = np.unique(cell_info)

bname = 'GCaMP8_exported_ROIs_s2f_full/dff_v02/'

# fitting codes
def run_single_cell(nf, bname=bname):
    nf = nf.squeeze()
    files = sorted(glob(f'GCaMP8_exported_ROIs_s2f_full/data_v01/{nf}*.npz'))
