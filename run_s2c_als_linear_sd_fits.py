...
def run_single_cell(nf):
    ...
    _=np.load('GCaMP8_exported_ROIs_s2f_full/fit_als_linear_sd_results_v04/'+nf+'_fit.npz', allow_pickle=True)
    param_kernel = _['param_kernel']
    
    for n in range(2): # do 4 times of ALS at 1st round, 2 times in 2nd
    ...
    np.savez('GCaMP8_exported_ROIs_s2f_full/fit_als_linear_sd_results_v05/'+nf+'_fit', param_linear=np.array(param_linear), param_kernel=np.array(param_kernel))
    return None
